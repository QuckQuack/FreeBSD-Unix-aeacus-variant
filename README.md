...
`aeacus` is a vulnerability scoring engine for Windows, Linux, and FreeBSD, with an emphasis on simplicity.
...
0. **Extract the release** into `/opt/aeacus` (Linux/FreeBSD) or `C:\aeacus\` (Windows).

	> Try compiling it yourself! Or, you can [download the releases here](https://github.com/elysium-suite/aeacus/releases).
	>
	> **FreeBSD users:** pre-built binaries aren't published as a release yet, so you'll need to build them yourself
	> for now -- it's one command (`make bsd`). See [Compiling from Source](docs/compiling.md) for full instructions.
...
Make sure you have a recent version of `go` installed, as well as `git` and `make`. If you want to compile Windows, Linux, and FreeBSD, install all dependencies using `go get -v -d -t ./...`. Then to compile, use `go build`, OR make:

- Building for `Linux`: `make lin`
- Building for `Windows`: `make win`
- Building for `FreeBSD`: `make bsd`
...
- Building for `FreeBSD`: `make bsd-dev`

### Releases

You can build release files (e.g., `aeacus-linux.zip`). These will have auto-randomized `crypto.go` files.

- Building all platforms: `make release`
- Building FreeBSD only: `make release-bsd`

**For full build requirements, cross-compiling details, release packaging, and troubleshooting, see
