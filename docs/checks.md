...
> **Note**: `PasswordChanged`, `FileOwner`, and `PermissionIs` are also available on Linux and FreeBSD (not listed
> separately here since their behavior and arguments are identical on both -- `PasswordChanged` reads
> `/etc/shadow`).
...
#### FreeBSD Bonus Checks

These checks have no Linux equivalent in `aeacus`, and take advantage of mechanisms that are unique (or much more
idiomatic) to FreeBSD.
