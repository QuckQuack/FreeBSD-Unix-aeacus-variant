...
func getDesc(c cond) string {
	...
	// FreeBSD checks
	case "SysctlValue":
		return "Sysctl " + c.Key + " is set to " + c.Value
	case "SysctlValueNot":
		return "Sysctl " + c.Key + " is not set to " + c.Value
	case "PkgAuditClean":
		return "No known package vulnerabilities (pkg audit)"
	case "PkgAuditCleanNot":
		return "Known package vulnerabilities are present (pkg audit)"
	case "RcVarEnabled":
		return "rc.conf variable " + c.Name + " is enabled"
	case "RcVarEnabledNot":
		return "rc.conf variable " + c.Name + " is not enabled"
	case "JailRunning":
		return "Jail " + c.Name + " is running"
	case "JailRunningNot":
		return "Jail " + c.Name + " is not running"
